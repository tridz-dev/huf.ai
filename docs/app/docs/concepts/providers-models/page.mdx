...
1. **Provider Name** — enter the AI service name.
   - Use standard names such as `OpenAI`, `Anthropic`, `Google`, or `OpenRouter`.
   - Names are case-insensitive and automatically routed to LiteLLM.
   - For non-standard providers, check the [LiteLLM docs](https://docs.litellm.ai/docs/providers).

2. **API Key** — paste the key from your provider account.
   - Get keys from the provider websites (links below).
   - Huf stores the key securely using Frappe's encrypted Password field.
   - The key is never exposed in logs or API responses.
...
1. **Model Name** — enter the model identifier.
   - Use user-friendly names such as `gpt-4-turbo` or `claude-3-opus`.
   - Or use LiteLLM format such as `openai/gpt-4-turbo`, which skips normalization.
...
2. **Provider** — select the provider you created earlier.
   ...
   - It must match the provider that actually offers this model.

3. **Save** the model record.
