...
1. **Navigate** to Agent Trigger and click **New**.
2. **Select Agent** from the dropdown.
3. **Choose Trigger Type**: **Doc Event** for document-based automation, or **Schedule** for time-based execution.
4. **Configure Details** for the chosen type (see the sections below).
5. **Set Prompt Template** so the agent knows what task to perform.
