...
<ChatTranscript>
  <ChatMessage role="user">What's the status of order SO-2024-001?</ChatMessage>
  <ChatMessage role="agent">The order is confirmed and pending shipment.</ChatMessage>
  <ChatMessage role="user">When will it ship?</ChatMessage>
  <ChatMessage role="agent">Which order are you asking about? (No memory)</ChatMessage>
</ChatTranscript>
...
<ChatTranscript>
  ...
  <ChatMessage role="agent">The order is confirmed and pending shipment.</ChatMessage>
  <ChatMessage role="user">When will it ship?</ChatMessage>
  <ChatMessage role="agent">Order SO-2024-001 is scheduled to ship on Jan 15. ✓ Remembers context</ChatMessage>
</ChatTranscript>
...
<ConfigPanel title="Shared Conversation Settings">
  <ConfigField label="Persist User History" value="0" />
</ConfigPanel>
...
<ChatTranscript>
  ...
  <ChatMessage role="agent">Sure! Let me get that information for you.</ChatMessage>
  <ChatMessage role="tool-call">get_customer(CUST-001)</ChatMessage>
  <ChatMessage role="tool-result">{'{...customer data...}'}</ChatMessage>
  <ChatMessage role="agent">Here's the information for ABC Corporation...</ChatMessage>
  <ChatMessage role="user">What's their outstanding balance?</ChatMessage>
  <ChatMessage role="agent">[Reads history, knows we're talking about CUST-001] Their current outstanding balance is $5,230.</ChatMessage>
</ChatTranscript>
...
<ConfigPanel title="Short Conversation Settings">
  <ConfigField label="Persist Conversation" value="0" />
  <ConfigField label="History Limit" value="Short" />
</ConfigPanel>
...
<ConfigPanel title="Long Conversation Settings">
  <ConfigField label="Persist Conversation" value="1" />
  <ConfigField label="History Limit" value="Long" />
  <ConfigField label="Cost Monitoring" value="Enabled" />
  <ConfigField label="Summarization Strategy" value="For very long threads" />
</ConfigPanel>
...
<ConfigPanel title="Session-Based Conversation Settings">
  <ConfigField label="History Persistence" value="Within session" />
  <ConfigField label="Auto-Expire" value="After timeout" />
  <ConfigField label="Session Boundaries" value="Defined" />
</ConfigPanel>
...
**Questions about conversations?** Check [GitHub discussions](https://github.com/tridz-dev/huf/discussions).
