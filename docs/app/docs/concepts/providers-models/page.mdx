...
1. **Create a Provider** with an API key.
2. **Create a Model** linked to that Provider.
3. **Assign the Model to an Agent**.
4. **When the agent runs**, Huf loads the provider credentials, calls LiteLLM with the normalized model name, LiteLLM routes the request to the correct provider API, and the response is returned to the agent.
